...
public class Answer5 {
    public static boolean isPalindrome(int number) {
        int reverse = 0;//reverse of given number initialize 0
        int n = Math.abs(number);//absolute value of number to avoid checking for negative number
        int length = String.valueOf(n).length();
        int i = 0;
        while (i < length) {
            int lastDigit = n % 10;
            reverse += lastDigit;//add last digit to reverse
            if (i != length - 1) reverse *= 10;
            n = n / 10; //to remove the last digit of num
            i++;//increment i to iterate through every digit
        }
        return reverse == Math.abs(number);//reverse of number must be = to number for number to be palindrome
    }
